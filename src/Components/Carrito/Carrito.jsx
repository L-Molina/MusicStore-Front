...
/**
 * Pantalla inspirada en `carrito_de_compra_musicstore_v6/code.html`
 * + barra inferior y footer sintetizados.
 */
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-8 pb-32 pt-32 text-center font-sans">
        <MaterialSymbol className="mb-4 text-6xl text-zinc-600">shopping_cart</MaterialSymbol>
        <h1 className="text-4xl font-extrabold uppercase tracking-tighter text-[#e2e2e2]">
          Tu Carrito está vacío
        </h1>
        <p className="mt-3 text-zinc-500">
          Explorá el catálogo y agregá instrumentos antes de finalizar tu compra.
        </p>
        <Link
          to="/catalogo"
            ...
            {RELATED_SUGGESTIONS.map((s) => (
              <div key={s.id} className="group cursor-pointer">
                <Link to="/catalogo" className="block">
                  ...
                  <p className="font-sans text-sm font-semibold text-[#e2e2e2]">{s.name}</p>
                  <p className="font-sans font-bold text-[#ba203f]">{formatPriceEUR(s.price)}</p>
                </Link>
              </div>
            ))}
          </div>
